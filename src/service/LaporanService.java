...
public class LaporanService {
    ...
    public LaporanModel getLaporanHarian(int idUser, String tanggal) {
        ...
        String sql
                = "SELECT "
                + "COALESCE(SUM(CASE WHEN c.tipe='Pemasukan' THEN t.jumlah ELSE 0 END),0) pemasukan, "
                + "COALESCE(SUM(CASE WHEN c.tipe='Pengeluaran' THEN t.jumlah ELSE 0 END),0) pengeluaran, "
                + "COUNT(t.id_transaksi) jumlah_transaksi "
                + "FROM transaksi t "
    ...
    public LaporanModel getLaporanBulanan(int idUser, String bulan) {
        ...
        try {
            YearMonth ym = YearMonth.parse(bulan);

            laporan.setTanggalAwal(bulan + "-01");
            laporan.setTanggalAkhir(ym.atEndOfMonth().toString());
